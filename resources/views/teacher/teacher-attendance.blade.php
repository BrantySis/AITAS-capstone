    ...
    <div class="p-4 grid grid-cols-1 gap-4">
        ...
        <div class="panel-1-outer relative">
            ...
            <div class="check-button-floating z-50">
                <form method="POST" action="{{ route('teacher.attendance.store') }}" id="attendance-form-{{ $schedule->id }}">
                    @csrf
...
function handleAttendanceAction(scheduleId, isCheckout, posData=null){
    const submit=(pos)=>{
        ...
        if(!isCheckout && finalLat && finalLng){
            ...
            if(dist>80){ showNotification("❌ You are too far! ("+Math.round(dist)+"m)",'error'); return; }
        }
...
function initMap(){
    ...
    if(navigator.geolocation){
        geoWatchId = navigator.geolocation.watchPosition(pos=>{
            ...
            document.getElementById('last-lat-{{ $schedule->id }}').value=uLat;
            document.getElementById('last-lng-{{ $schedule->id }}').value=uLng;
...
document.addEventListener('DOMContentLoaded', ()=>{
    initMap();
    document.getElementById('action-btn-{{ $schedule->id }}')?.addEventListener('click', () => {
        const isCheckout = {{ $is_checked_in_active ? 'true' : 'false' }};
        openFaceScanner({{ $schedule->id }}, isCheckout);
    });
});
</script>
@endif
