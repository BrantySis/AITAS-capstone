...
<div class="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
    ...
    <div class="mb-6">
        <h1 class="text-3xl font-extrabold text-gray-900 flex items-center">
            <svg class="w-8 h-8 mr-3 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.5v11M17.5 12h-11M3 7v10a2 2 0 002 2h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2z"/>
            </svg>
            Subject Catalog
        </h1>
    </div>
    ...
    {{-- This main div aligns the Filter/Search Form (left) and the Action Buttons (right) --}}
    <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-6 mb-8">
        
        {{-- LEFT COLUMN: SEARCH & FILTERS (Takes up most space, but search bar is shrunk) --}}
        <form method="GET" action="{{ route('admin.subjects.index') }}" 
            ...
            {{-- 2.1. Search Input (Reduced Width) --}}
            ...
            {{-- 2.2. Department Filter & Reset (Right of Search) --}}
            <div class="flex items-center gap-4 flex-wrap">
                
                {{-- Department Filter --}}
                @if(isset($departments) && $departments->count() > 0)
               <div class="flex-shrink-0 w-full md:w-56 **relative**">
                    <select name="department" id="department"
                        class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 bg-white **appearance-none** transition"
                ...
                @if(request('department') || request('search'))
                    <a href="{{ route('admin.subjects.index') }}"
                        class="text-center bg-gray-100 text-gray-700 px-5 py-2.5 rounded-xl hover:bg-gray-200 text-sm transition font-semibold flex-shrink-0">
                        Reset
                    </a>
                @endif
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        @foreach($deptSubjects as $subject)
                            <div class="bg-white p-6 shadow-xl rounded-2xl border border-gray-100 flex flex-col justify-between hover:ring-2 hover:ring-blue-500 transition duration-200">
                                <div class="mb-4">
                                    <p class="text-xs font-bold text-blue-600 tracking-widest">SubjectCode: {{ $subject->subject_code }}</p>
...
<div id="create-subject-modal" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-75 backdrop-blur-sm transition-opacity duration-300 modal-wrapper" aria-hidden="true">
    {{-- MODAL CENTERING CONTAINER: Uses grid to center the modal box vertically and horizontally --}}
    <div class="absolute top-0 left-0 w-full h-full grid place-items-center p-4 overflow-y-auto">
        <div class="bg-white rounded-2xl shadow-2xl transform transition-all sm:max-w-lg w-full scale-95 opacity-0 duration-300" id="createSubjectModalContent">
                ...
                <form method="POST" action="{{ route('admin.subjects.store') }}">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="subject_code" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required placeholder="e.g., CS-101">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Units</label>
                            <input type="number" name="units" min="1" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required placeholder="e.g., 3">
                        </div>
                    </div>

                    <div class="mt-5">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Subject Name</label>
                        <input type="text" name="subject_name" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required placeholder="e.g., Introduction to Programming">
                    </div>
                    ...
                    <div class="mt-5">
                        ...
                        <input type="text" name="department" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" placeholder="e.g., Computer Science">
                    </div>

                    <div class="grid grid-cols-3 gap-5 mt-5">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Course Year</label>
                            <select name="course_year" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required>
                        ...
                        <div>
                            ...
                            <select name="semester" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required>
                        ...
                        <div>
                            ...
                            <select name="school_year" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required>
                    ...
                    <div class="mt-5">
                        ...
                        <textarea name="description" rows="3" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" placeholder="Briefly describe the subject's focus."></textarea>
                    </div>

                    <div class="mt-8 flex justify-end space-x-3 border-t border-gray-100 pt-5">
                        <button type="button" data-modal-close="create-subject-modal" class="px-5 py-2.5 border-2 border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 transition font-semibold">Cancel</button>
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-semibold shadow-md transition">Create Subject</button>
                    </div>
                </form>
            </div>
        </div>
...
{{-- EDIT SUBJECT MODAL --}}
<div id="edit-subject-modal" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-75 backdrop-blur-sm transition-opacity duration-300 modal-wrapper" aria-hidden="true">
    {{-- MODAL CENTERING CONTAINER: Uses grid to center the modal box vertically and horizontally --}}
    <div class="absolute top-0 left-0 w-full h-full grid place-items-center p-4 overflow-y-auto">
        <div class="bg-white rounded-2xl shadow-2xl transform transition-all sm:max-w-lg w-full scale-95 opacity-0 duration-300" id="editSubjectModalContent">
            <div class="p-6 sm:p-8">
                ...
                <form method="POST" id="edit-subject-form">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Subject Code</label>
                            <input type="text" name="subject_code" id="edit_subject_code" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Units</label>
                            <input type="number" name="units" id="edit_units" min="1" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required>
                        </div>
                    </div>

                    <div class="mt-5">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Subject Name</label>
                        <input type="text" name="subject_name" id="edit_subject_name" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required>
                    </div>

                    <div class="grid grid-cols-3 gap-5 mt-5">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Course Year</label>
                            <select name="course_year" id="edit_course_year" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Semester</label>
                            <select name="semester" id="edit_semester" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">School Year</label>
                            <select name="school_year" id="edit_school_year" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" required>
                    ...
                    <div class="mt-5">
                        ...
                        <input type="text" name="department" id="edit_department" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition" placeholder="e.g., Computer Science">
                    </div>

                    <div class="mt-5">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                        <textarea name="description" id="edit_description" rows="3" class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition"></textarea>
                    </div>

                        <div class="mt-8 flex justify-end space-x-3 border-t border-gray-100 pt-5">
                        <button type="button" data-modal-close="edit-subject-modal" class="px-5 py-2.5 border-2 border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 transition font-semibold">Cancel</button>
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-semibold shadow-md transition">Update Subject</button>
                    </div>
                </form>
            </div>
        </div>
...
<div id="import-subjects-modal" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-75 backdrop-blur-sm transition-opacity duration-300 modal-wrapper" aria-hidden="true">
    <div class="absolute top-0 left-0 w-full h-full grid place-items-center p-4 overflow-y-auto">
        <div class="bg-white rounded-2xl shadow-2xl transform transition-all sm:max-w-lg w-full scale-95 opacity-0 duration-300" id="importSubjectsModalContent">
            <div class="p-6 sm:p-8">
                <h3 class="text-2xl font-bold text-gray-900 mb-2">Import Subjects</h3>
                <p class="text-sm text-gray-500 mb-6">Upload a CSV or Excel file to bulk import subjects. Please follow the instructions below carefully:</p>

                <ul class="list-disc list-inside text-gray-700 text-sm mb-6">
                    <li>The file must be in <strong>CSV</strong>, <strong>XLS</strong>, or <strong>XLSX</strong> format.</li>
                    <li>Columns must exactly match: 
                        <code>subject_code, subject_name, department, course_year, semester, school_year, units, description, is_active</code>.</li>
                    <li><strong>Required fields:</strong> <em>subject_code, subject_name, units</em>. All other columns are optional.</li>
                    <li><strong>is_active</strong> should be <code>1</code> for active subjects (default is 1 if left blank).</li>
                    <li>Ensure <strong>subject_code</strong> is unique to avoid import errors.</li>
                    <li>You can <a href="{{ route('admin.subjects.downloadTemplate') }}" class="text-blue-600 hover:underline">download the template CSV here</a>.</li>
                </ul>
                ...
                <form method="POST" action="{{ route('admin.subjects.processImport') }}" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <input type="file" name="file" accept=".csv,.xlsx,.xls" required
                            class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-blue-500 focus:border-blue-500 transition">
                        @error('file')
                    ...
                    <div class="mt-8 flex justify-end space-x-3 border-t border-gray-100 pt-5">
                        <button type="button" data-modal-close="import-subjects-modal" class="px-5 py-2.5 border-2 border-gray-300 rounded-lg text-gray-700 hover:bg-gray-100 transition font-semibold">Cancel</button>
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg font-semibold shadow-md transition">Import Subjects</button>
                    </div>
                </form>
            </div>
        </div>
